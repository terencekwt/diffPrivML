...
For example, to fit a model of **DPNaiveBayesClassifier**, the privacy budget, epsilon, can be specified as part of the input.
...
All the ML algorithms in this package so far uses output perturbation methods.  
...
Note this is only supported for categorical data. DPNaiveBayesClassifier, on the other hand, support for mixed categorical / numerical attributes.
...
accuracy <- sapply(eps, function(epsilon) {
  rdtDP <- DPRandomDecisionTreeClassifier(Y = trainingSetY, X = trainingSetX, height = 3, numTrees = 6)
  predictions <- predict(rdtDP, testSetX)
